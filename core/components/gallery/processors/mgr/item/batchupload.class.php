 ...
 */
class GalleryItemBatchUploadProcessor extends modProcessor {
    ...
    public function process(){
        ...
        if ($this->modx->getVersionData()['version'] >= 3){
            //V3
            $filehandler = new MODX\Revolution\File\modFileHandler($modx);
        } else {
            //V2
            $modx->getService('fileHandler','modFileHandler');
            $filehandler = $modx->fileHandler;
        }
        $fullpath = $filehandler->sanitizePath($directory);
...
return 'GalleryItemBatchUploadProcessor';
